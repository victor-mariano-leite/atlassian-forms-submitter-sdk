...
from atlassianforms.form.manager import ServiceDeskFormManager, ServiceDeskFormFilled, ServiceDeskForm, ServiceDeskFormField, ServiceDeskFormFieldValue
...
def test_list_field_values_invalid_field(form_manager):
    with pytest.raises(ValueError, match="Field with identifier 'invalid_field' not found."):
...
def test_validate_invalid_choice(form_manager):
    ...
    with pytest.raises(ValueError, match="Invalid choice value 'invalid_priority' for field 'Priority' or 'priority'."):
        form_manager.validate(filled_values)
